...
/** FAICON **/
MySQL.prototype.sum = function(model, where, options, cb) {
  ...
  let select = options.fields.map(field => 'sum(`' + field + '`) as "' + field + '"');

  var stmt = new ParameterizedSQL('SELECT ' + select.join(',') + ' FROM ' + this.tableEscaped(model));
  ...
  this.execute(stmt.sql, stmt.params, options, function(err, res) {
    ...
    var c = (res && res[0]) || 0;
    // Some drivers return sum as a string to contain bigint
